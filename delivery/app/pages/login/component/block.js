...
class ViewRegister extends Component {
    ...
    render() {
        return (
            <View style={styleRegitser.register}>
                ...
                <View style={styleRegitser.viewPhoneAndPw} >
                    <Text style={styleRegitser.textPhoneAndPw} >验证码</Text>
                    <TextInput style={styleRegitser.inputSms} placeholder="输入验证码" keyboardType="number-pad" maxLength={6} onChangeText={(text) => this.state.smsCode = text} />
                    <View style={styleRegitser.buttonSms}>
                        <Button title={this.state.liked == false ? "获取" : this.state.c} disabled={this.state.liked} onPress={() => this.getSmsCode()} />
                    </View>
                </View>

                <View style={styleRegitser.viewPhoneAndPw}>
                    <Text style={styleRegitser.textPhoneAndPw}>密码</Text>
                    <TextInput style={styleRegitser.inputPw} secureTextEntry={true} placeholder="输入密码" autoCapitalize="none" maxLength={16} onChangeText={(text) => this.state.passWord = text} />
                </View>
                <View style={styleRegitser.buttonRegister} >
                    <Button color="white" title="注册" onPress={() => this.register()} />
                </View>
    ...
    register = () => {
        ...
        if (this.inputValidation()) {
            ...
            this.store.setLoginInfo(this.state.phoneNumber,this.state.passWord);
            //发送请求
            let data = { phoneNumber: this.state.phoneNumber,password:this.state.passWord,smsCode:this.state.smsCode };
            sendWs("register_account", data);
        }
    }
}
