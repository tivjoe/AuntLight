...
export class LoginStore {
    ...
    userInfoStorage=(id,token)=>{
        storage.save({
            key: 'deliveryInfo', // 注意:请不要在key中使用_下划线符号!
            data: {
                id:id,
                token:token
            },
     ...
     * 接收网络请求结果接口
     * 处理登陆
     * @param int status 0:失败 1:成功 2:未注册 3:密码或者账号错误
     */
    ...
    WsRegister = (status,id,token) => {
        ...
        } else if (status === "have_phone") {
            Alert.alert("提示", "当前手机号已经注册")
